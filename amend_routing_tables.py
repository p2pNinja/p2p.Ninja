...
def amend_routing_tables (packet, returnaddr, unsigned_routing_dict, signed_routing_dict, unsigned_destination_list, signed_destination_list, settings):
    ...
    if (signature!=0) & (packet['type']=='broadcast') & (originid in signed_destination_list):
    ...
    if (signature!=0) & (packet['type']=='broadcast') : #new sender not known before
        ...
        if len(signed_destination_list)>settings['NUMBEROFKEYSTOSAVE']:
            ...
            signed_routing_dict.remove(signed_destination_list[0])
    ...
    if signature == 0:    # this always causes amending of the dictionary 
    ...
    if signature!=0 & (originid not in signed_destination_list):
